...
import { MapPin, IndianRupee, Sprout, CloudRain } from 'lucide-react';
...
export const Hero: React.FC<HeroProps> = ({ language }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      particles.push({
        ...
        r: Math.random() * 2 + 0.5,
        // Reduced velocity for smoother, slower movement
    ...
    const animate = () => {
      ...
      particles.forEach(p => {
        ...
        ctx.fillStyle = `rgba(176, 139, 90, ${p.alpha})`; // Bronze color
        ctx.fill();
      });
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-center items-center pt-20 pb-32 overflow-hidden">
      ...
      {/* Glow effects - refined for both modes */}
      ...
      <div className="relative z-10 text-center px-4 max-w-5xl mx-auto space-y-8">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-emerald-900/10 dark:border-white/10 bg-white/60 dark:bg-white/5 backdrop-blur-md mb-4 shadow-sm">
        ...
        <h1 className="text-5xl md:text-8xl font-outfit font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-b from-gray-900 to-gray-600 dark:from-white dark:to-white/60 drop-shadow-sm pb-2">
          {t.heroTitle}
        </h1>
        
        <p className="text-xl md:text-2xl font-inter font-light text-gray-600 dark:text-gray-400">
          {t.heroSub}
        </p>

        <p className="max-w-2xl mx-auto text-gray-500 dark:text-gray-500 font-inter leading-relaxed text-lg">
          {t.desc}
        </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full mt-12">
          {[
            ...
            { 
              icon: IndianRupee, 
              label: 'Mandi Rates', 
              color: 'from-amber-50 to-white dark:from-amber-900/50 dark:to-amber-800/20', 
              hover: 'from-amber-100 to-amber-50 dark:from-amber-600/30 dark:to-amber-900/80',
              iconColor: 'text-amber-800 dark:text-white/70',
              action: 'View',
              targetId: 'mandi-rates'
            },
            { 
              icon: CloudRain, 
              label: 'Weather', 
            ...
            <button 
              ...
              className={`group relative overflow-hidden rounded-3xl border border-white dark:border-white/5 bg-gradient-to-br ${item.color} p-6 backdrop-blur-sm transition-all duration-500 hover:border-gold hover:-translate-y-2 shadow-lg shadow-gray-200/50 dark:shadow-black/20 hover:shadow-xl cursor-pointer`}
            >
      ...
      {/* Weather Strip */}
      <div className="absolute bottom-0 w-full bg-white/60 dark:bg-white/5 backdrop-blur-md">

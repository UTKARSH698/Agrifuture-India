...
export const MandiTicker: React.FC<MandiTickerProps> = ({ userLocation }) => {
  ...
  const sortedRates = React.useMemo(() => {
    if (!userLocation) return MANDI_RATES;
    
    const local = MANDI_RATES.filter(r => r.state === userLocation.state);
    const others = MANDI_RATES.filter(r => r.state !== userLocation.state);
